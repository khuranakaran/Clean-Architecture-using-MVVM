...
object DataHelpers {
    ...
    val normalHtmlData: String
        get() = ("<p>\n"
                ...
                + "    ©1957, Meridian Books.\n"
                + "</p>")
    val mathQuestionWithHtmlTags: String
        get() {
            return ("<p>\n"
    ...
    fun getFormulas(context: Context): List<String> {
        val formulas: MutableList<String> =
            ArrayList(Arrays.asList(*context.resources.getStringArray(R.array.formulas)))
        formulas.add(mathWithDisplayStyle)
        formulas.add(simpleMath)
        formulas.add(plainTextWithMath)
        formulas.add(normalHtmlData)
        formulas.add(mathQuestionWithHtmlTags)
        Log.d(TAG, formulas.toTypedArray().toString())
        return formulas
    }

    @JvmName("getScrollableData1")
